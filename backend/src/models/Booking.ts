...
bookingSchema.pre('save', async function(next) {
  try {
    ...
    if (this.isNew) {
      ...
      const isDateAvailable = tour.startDates.some((date: Date) => 
...
bookingSchema.statics.getMonthlyRevenue = async function(year: number) {
...
bookingSchema.statics.getPopularTours = async function(limit: number = 10) {
...
bookingSchema.statics.cancelBooking = async function(
  ...
  const daysDifference = Math.ceil(
    (booking.startDate.getTime() - new Date().getTime()) / (1000 * 3600 * 24)
  );
