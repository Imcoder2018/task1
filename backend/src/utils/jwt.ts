 ...
 * Returns the decoded payload if valid, throws error if invalid
 */
export const verifyRefreshToken = (token: string): Omit<ITokenPayload, 'role'> => {
  const jwtRefreshSecret = process.env.JWT_REFRESH_SECRET || process.env.JWT_SECRET;
 ...
 */
export const getTokenTimeToLive = (token: string): number => {
  try {
    const decoded = decodeToken(token);
...
export const refreshAccessToken = async (
  refreshToken: string,
  getCurrentUserRole: (userId: string) => Promise<string>
): Promise<string> => {
